...
public class ConfigInfoMapperByOracle extends AbstractMapperByOracle implements ConfigInfoMapper {
    ...
    public MapperResult findConfigInfoByAppFetchRows(MapperContext context) {
        ...
        String sql = "SELECT * FROM (SELECT id,data_id,group_id,tenant_id,app_name,content, ROWNUM as rnum FROM config_info"
                + " WHERE tenant_id LIKE ? AND app_name= ?)" + " WHERE  rnum >= " + (context.getStartRow() + 1) + " and "
                + (context.getPageSize() + context.getStartRow()) + " >= rnum";
    ...
    public MapperResult getGroupIdList(MapperContext context) {
        String sql = "SELECT * FROM (SELECT group_id, ROWNUM as rnum FROM config_info WHERE tenant_id ='" + NamespaceUtil.getNamespaceDefaultId()
                + "' GROUP BY group_id) " + " WHERE  rnum >= " + (context.getStartRow() + 1) + " and "
                + (context.getPageSize() + context.getStartRow()) + " >= rnum";
        return new MapperResult(sql, Collections.emptyList());
    }
    ...
    public MapperResult findAllConfigInfoFragment(MapperContext context) {
        ...
        String sql = "SELECT * FROM (SELECT id,data_id,group_id,tenant_id,app_name," + (needContent ? "content," : "")
                ...
                + ", ROWNUM as rnum FROM config_info WHERE id > ? ORDER BY id ASC) " + " WHERE  rnum >= " + (context.getStartRow() + 1) + " and "
                + (context.getPageSize() + context.getStartRow()) + " >= rnum ";
    ...
    public MapperResult findConfigInfoBaseLikeFetchRows(MapperContext context) {
        ...
        return new MapperResult(sqlFetchRows + where + ") " + " WHERE  rnum >= " + (context.getStartRow() + 1) + " and "
                + (context.getPageSize() + context.getStartRow()) + " >= rnum ",
                paramList);
    }
    ...
    public MapperResult findConfigInfo4PageFetchRows(MapperContext context) {
        ...
        return new MapperResult(sql + where + ")  " + " WHERE  rnum >= " + (context.getStartRow() + 1) + " and "
                + (context.getPageSize() + context.getStartRow()) + " >= rnum ",
                paramList);
    }
